...
def create_track(track: TrackModel, session: Session = Depends(get_session)):
    session.add(track)
    session.commit() # actually create row and id
    session.refresh(track) # needed to get this new data accessible to python app
    return track
...
def update_track(track_id: int, updated_track: Track, response: Response, session: Session = Depends(get_session)):
    ...
    track_dict = updated_track.dict(exclude_unset=True) # To not send id created by pydantic 
